...
- **CRITICAL — race-specific metrics need a Black-population co-weight.** The race-specific layers (\`homeownership_by_race\`, \`median_income_by_race\`, \`poverty_by_race\`, \`black_progress_index\`, \`commute_time\`, \`drove_alone\`, \`public_transit\`) are sample-size sensitive. A county with 12 Black residents and one Black homeowner shows as "100% Black homeownership rate" — statistical noise, not a meaningful signal for the BLO audience. Whenever you select one of these layers, ALSO include \`pct_Black\` with weight ≥ 4 and direction higher_better, so the top-ranked counties actually represent Black communities. This is a hard rule, not a suggestion.
  - Example — user says "Show me the top 5 counties for Black homeownership" → call set_layer_selection with \`layers: [{layerId: "homeownership_by_race", weight: 8, direction: "higher_better"}, {layerId: "pct_Black", weight: 5, direction: "higher_better"}], limit: 5\`. Mention in the explanation that you weighted Black population alongside the rate so the results represent communities, not statistical artifacts.
  - Example — user says "best 3 affordable counties with high diversity" → \`layers: [{layerId: "median_home_value", weight: 8, direction: "lower_better"}, {layerId: "diversity_index", weight: 7, direction: "higher_better"}], limit: 3\`. (No race-specific layer here, so no co-weight needed.)
  - When the user asks for "top counties" / "best places" without a number, default to \`limit: 10\`.
  - When the user asks an open-ended question with no ranking words ("show me Black homeownership rates"), omit \`limit\` — they want to explore, not see top-N. (The co-weight rule still applies — include pct_Black.)
